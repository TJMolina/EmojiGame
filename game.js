...
if(localStorage.getItem('record_time')<200){localStorage.clear()}
}
showRecord();
...
const Dibujar=(objeto,x,y)=>game.fillText(emojis[objeto],x,y);
...
function setCanvasSize(){
     if(elementsSize)tamanioAntig = elementsSize;
...
function cambiodezona(){
    ...
    canvas.style.backgroundColor='#feff9d';
        ...
        WhatPlayer='PLAYER2';
        WhatEnemy = '2';
        canvas.style.backgroundColor='#57bbf5';
    }
}
...
function imprimirMapa(mapRows,mapRowsCols,map){
    ...
    mapRowsCols.forEach((row,rowI)=>{
        row.forEach((col,colI)=>{
            ...
            Dibujar(col,posX,posY);
           if(pause&&col=='O'){Dibujar(WhatPlayer,playerPosition.x,playerPosition.y)}
...
function movePlayer(){
        ...
        function colisionConJugador(X,Y){
            const x = playerPosition.x.toFixed(0) == X;
            const y = playerPosition.y.toFixed(0) == Y;
            return x&&y;
        }

    const giftCollision = colisionConJugador(giftPosition.x.toFixed(0),giftPosition.y.toFixed(0));
    if(giftCollision){levelWin()}

    const enemyCollision = enemyPosition.find(enemy=>{
        return colisionConJugador(enemy.x.toFixed(0),enemy.y.toFixed(0));
    });
    if(enemyCollision){deadAnimation();}
    
    if(instakill.x){
        const instakillCollision = colisionConJugador(instakill.x.toFixed(0),instakill.y.toFixed(0));
        if(instakillCollision){lives=0;deadAnimation()}
    }

    Dibujar(WhatPlayer,playerPosition.x,playerPosition.y);
}
...
function deadAnimation(){
    if(WhatEnemy=='X'){
        ...
        else WhatPlayer='BOMB_COLLISION';
    }
    else if(WhatEnemy=='2')WhatPlayer = 'BOMB_COLLISION2'
    clearInterval(timeInterval);
    pause = true;
